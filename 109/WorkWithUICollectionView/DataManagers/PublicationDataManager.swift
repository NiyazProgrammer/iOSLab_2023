...
class PublicationDataManager: PublicationProtocol {
            ...
            for (userKey, publications) in self.dictionaryUserAndPublication {
                if let index = publications.firstIndex(where: { $0.id == publicationId }) {
                    ...
                    if publication.likes.contains(userName) {
                        publication.likes.removeAll { $0 == userName }
                    } else {
                        publication.likes.append(userName)
                    }
                    self.dictionaryUserAndPublication[userKey]?[index] = publication
                    self.savePosts()
                    self.loadPosts()
                }
            }
    ...
    private func savePosts() {
        do {
            let jsonData = try JSONEncoder().encode(dictionaryUserAndPublication)
            userDefaults?.set(jsonData, forKey: "userPosts")
        } catch {
            print("Ошибка при сохранении постов в UserDefaults: \(error)")
        }
    }

    private func loadPosts() {
        if let jsonData = userDefaults?.data(forKey: "userPosts") {
            do {
                let loadedPosts = try JSONDecoder().decode([String: [Publication]].self, from: jsonData)
                dictionaryUserAndPublication = loadedPosts
            } catch {
                print("Ошибка при загрузке постов из UserDefaults: \(error)")
            }
        }
    }
}
